...
describe('makeSlug', () => {
  it('converts a name to a lowercase slug with random suffix', () => {
    ...
    expect(slug).toMatch(/^the-alvarez-family-[a-z0-9]{8}$/);
  });

  it('trims whitespace', () => {
    const slug = makeSlug('  Manuela  ');
    expect(slug).toMatch(/^manuela-[a-z0-9]{8}$/);
  });

  it('replaces non-alphanumeric characters with hyphens', () => {
    const slug = makeSlug('Sam & Priya');
    expect(slug).toMatch(/^sam-priya-[a-z0-9]{8}$/);
  });
  ...
  it('strips leading and trailing hyphens', () => {
    ...
    expect(slug).toMatch(/^hello-[a-z0-9]{8}$/);
  });
